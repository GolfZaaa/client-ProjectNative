import { Animated, LayoutAnimation, StyleSheet, Text, TextInput, TouchableOpacity, View } from "react-native";
...
const CreateNewPasswordScreen = ({navigation,route}:any) => {
  ...
  const HandleChangePassword = async () => {
    const test = await dispatch(CreateNewPassword({ email: email,password:password,confirmPassword:confirmPassword }) as any)

    if(test.payload.value.message === "Password and confirm password do not match")
    {
  ...
  return (
    ...
    >
     ...
     <TextInput
        ...
        />
        {password.length > 0 && (
          <TouchableOpacity onPress={togglePasswordVisibility} style={{paddingRight:20}}>
